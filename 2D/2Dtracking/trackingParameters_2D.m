function trackParam = trackingParameters_2D
%TRACKINGPARAMETERS_2D returns tracking parameters, from uTrack-2.1.1
%by KJaqaman, 'scriptTrackGeneral'
%   EHarry Nov 2014
...
parameters.minSearchRadius = 0.2; %minimum allowed search radius. The search radius is calculated on the spot in the code given a feature's motion parameters. If it happens to be smaller than this minimum, it will be increased to the minimum.
...
parameters.brownStdMult = 3.5; %multiplication factor to calculate search radius from standard deviation.
...
parameters.minSearchRadius = 0.4; %minimum allowed search radius.
parameters.maxSearchRadius = 4; %maximum allowed search radius.
parameters.brownStdMult = 3.5*ones(gapCloseParam.timeWindow,1); %multiplication factor to calculate Brownian search radius from standard deviation.
...
parameters.linStdMult = 3.5*ones(gapCloseParam.timeWindow,1); %multiplication factor to calculate linear search radius from standard deviation.
...
parameters.maxAngleVV = 50; %maximum angle between the directions of motion of two tracks that allows linking them (and thus closing a gap). Think of it as the equivalent of a searchRadius but for angles.
...
trackParam.costMatrices = costMatrices;
trackParam.gapCloseParam = gapCloseParam;
trackParam.kalmanFunctions = kalmanFunctions;
