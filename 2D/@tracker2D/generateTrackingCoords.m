function tracks = generateTrackingCoords( tracks, cellCoords )
%GENERATETRACKINGCOORDS generates tracks with specified coords
%   EHarry Nov 2014

type = size(cellCoords,2);
switch type
    case 1
        nDim = 3;
        tracks = placeCoords(tracks,cellCoords);
    case 2
        nDim = 2;
        for i = 1:2
            tracks(i).trackInfo = placeCoords(tracks(i).trackInfo,cellCoords(:,i));
        end
end
