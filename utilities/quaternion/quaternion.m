...
%         modified 25 July 2012, enforce normalization of q0 and q1
    ...
    function vp = RotateVectorCross( varargin )
% function vp = RotateVectorCross( q, v, dim ) or
%          vp = q.RotateVectorCross( v, dim )
...
%  vp       3xN or Nx3 element rotated vectors
        ...
        if neq == nev
            ...
            for iel = 1 : neq
                r = q(iel).e(2:4);
                w = q(iel).e(1);
                vp(:,iel)   = v(:,iel) + cross(2.*r,(cross(r,v(:,iel)) + w.*v(:,iel)));
            end
            if dim > 1
                vp  = ipermute( vp, perm );
            end
        elseif nev == 1
            ...
            for iel = 1 : neq
                r = q(iel).e(2:4);
                w = q(iel).e(1);
                vp(:,iel)   = v + cross(2.*r,(cross(r,v) + w.*v));
            end
            if siz(2) == 1
                vp  = squeeze( vp );
            end
        elseif neq == 1
            r = repmat(q.e(2:4),1,nev);
            w = q.e(1);
            vp  = v + cross(2.*r,(cross(r,v) + w.*v));
            vp  = reshape( vp, sip );
            if dim > 1
                vp  = ipermute( vp, perm );
            end
...
%  q        quaternion array
    ...
    function q = rotationmatrixEigen( R )
        ...
        d = zeros(4,nel);
        for iel = nel : -1 : 1
            d(:,iel) = RotMat2eEigen( chop( R(:,:,iel) ));
        end
        q   = quaternion( d(1,:), d(2,:), d(3,:), d(4,:) );
        q   = normalize( q );
...
function eout = RotMat2eEigen( R )
% function eout = RotMat2eEigen( R )
% One Rotation Matrix -> one quaternion
eout    = zeros(4,1);
if ~all( all( R == 0 ))
    r = R';
    R = ([ r(1,1) - r(2,2) - r(3,3) , r(2,1) + r(1,2) , r(3,1) + r(1,3) , ...
        r(2,3) - r(3,2); r(2,1) + r(1,2) , r(2,2) - r(1,1) - r(3,3) , r(3,2) + ...
        r(2,3) , r(3,1) - r(1,3) ; r(3,1) + r(1,3) , r(3,2) + r(2,3) , r(3,3) - ...
        r(1,1) - r(2,2) , r(1,2) - r(2,1) ; r(2,3) - r(3,2) , r(3,1) - r(1,3) , ...
        r(1,2) - r(2,1) , r(1,1) + r(2,2) + r(3,3) ]) ./ 3;
    [v,d] = eig(R);
    d = diag(d);
    [~,idx] = max(d);
    eout = v(:,idx);
