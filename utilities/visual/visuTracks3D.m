function visuTracks3D( trackedFeatureInfo, useMatlabVersion )
...
%% LOCAL FUNCTIONS
    ...
    function plotTracksLocal(figH,initialPlot)
        ...
        if initialPlot % initially set max range
            ...
            maxZ = max(imageInfo.tracksZ(:));
            minZ = min(imageInfo.tracksZ(:));
            set(gca,{'xlim','ylim','zlim'},{[minX maxX] , [minY maxY] , [minZ maxZ]})
            hold on
        end
        
        L = get(gca,{'xlim','ylim','zlim'});
        plot3(0,0,0,'marker','none','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
        set(figH,'Name',sprintf('Frame = %2d / %2d',currentTimePoint, timeInfo.nTimePoints));
        set(gca,{'xlim','ylim','zlim'},L)
        %         if ~initialPlot
        %             zoom reset
        %             set(gca,{'xlim','ylim'},L)
        %         end
        ...
        tracksZP = imageInfo.tracksZ(timeRange(1):timeRange(2),:);
        ...
        switch colorTime
            ...
            case '1' %if user wants to color-code time
                ...
                zData=arrayfun(@(x)[tracksZP(~isnan(tracksZP(:,x)),x); NaN],1:size(tracksZP,2),'Unif',false);
                plot3(vertcat(xData{:}),vertcat(yData{:}),vertcat(zData{:}),'k:','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                ...
                for i=1:numTimePlot-1
                    ...
                    zData=vertcat(tracksZP(i:i+1,validData),NaN(1,sum(validData)));
                    plot3(xData(:),yData(:),zData(:),'color',colorOverTime(i,:),'YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                end
            ...
            case '2' %no time color-coding, loop through series of colors to color tracks
                ...
                for i = 1 : trackStartRow(end) + numSegments(end) - 1
                    ...
                    plot3(tracksXP(obsAvail,i),tracksYP(obsAvail,i),tracksZP(obsAvail,i),'k:','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                    plot3(tracksXP(:,i),tracksYP(:,i),tracksZP(:,i),'color',colorLoop(mod(i-1,7)+1,:),...
                        'marker',markerType,'YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                end
            ...
            case '3' % no time color-coding, use extendedColors
                ...
                for i = 1 : trackStartRow(end) + numSegments(end) - 1
                    obsAvail = find(~isnan(tracksXP(:,i)));
                    plot3(tracksXP(obsAvail,i),tracksYP(obsAvail,i),tracksZP(obsAvail,i),'k:','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                    plot3(tracksXP(:,i),tracksYP(:,i),tracksZP(:,i),'color',imageInfo.extendedColors(mod(i-1,23)+1,:),...
                        'marker',markerType,'YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                    % put marker at current position
                    plot3(tracksXP(end,i),tracksYP(end,i),tracksZP(end,i),'color',imageInfo.extendedColors(mod(i-1,23)+1,:),...
                        'marker','+','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                end
            ...
            otherwise %no time color-coding, all tracks same color
                ...
                for i = 1 : imageInfo.trackStartRow(end) + imageInfo.numSegments(end) - 1
                    obsAvail = find(~isnan(tracksXP(:,i)));
                    plot3(tracksXP(obsAvail,i),tracksYP(obsAvail,i),tracksZP(obsAvail,i),'k:','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                    plot3(tracksXP(:,i),tracksYP(:,i),tracksZP(obsAvail,i),colorTime,'marker',markerType,'YLimInclude','off','XLimInclude','off','ZLimInclude','off');
        ...
        if imageInfo.mergeSplit
            ...
            for iTrack = 1 : imageInfo.numTracks
                ...
                for iSplit = indxSplit
                    ...
                    plot3([imageInfo.tracksX(timeSplit,rowS) imageInfo.tracksX(timeSplit-1,rowSp)], ...
                        [imageInfo.tracksY(timeSplit,rowS) imageInfo.tracksY(timeSplit-1,rowSp)],[imageInfo.tracksZ(timeSplit,rowS) imageInfo.tracksZ(timeSplit-1,rowSp)],'k-.','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                ...
                for iMerge = indxMerge
                    ...
                    plot3([imageInfo.tracksX(timeMerge-1,rowE) imageInfo.tracksX(timeMerge,rowM)], ...
                        [imageInfo.tracksY(timeMerge-1,rowE) imageInfo.tracksY(timeMerge,rowM)],[imageInfo.tracksZ(timeMerge-1,rowE) imageInfo.tracksZ(timeMerge,rowM)],'k--','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
            ...
            if imageInfo.mergeSplit
                ...
                for iTrack = 1 : imageInfo.numTracks
                    ...
                    for i = 1 : length(indxStart)
                        ...
                        startInfo(i,:) = [imageInfo.tracksX(timeStart,rowS) imageInfo.tracksY(timeStart,rowS) imageInfo.tracksZ(timeStart,rowS) timeStart];
                    ...
                    for i = 1 : length(indxEnd)
                        ...
                        endInfo(i,:) = [imageInfo.tracksX(timeEnd,rowE) imageInfo.tracksY(timeEnd,rowE) imageInfo.tracksZ(timeEnd,rowE) timeEnd];
                    ...
                    switch colorTime
                        case {'1','2','3'}
                            if ~isempty(startInfo)
                                plot3(startInfo(:,1),startInfo(:,2),startInfo(:,3),'k',...
                                    'LineStyle','none','marker','o','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                            end
                            if ~isempty(endInfo)
                                plot3(endInfo(:,1),endInfo(:,2),endInfo(:,3),'k',...
                                    'LineStyle','none','marker','square','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                            end
                ...
                for i=imageInfo.numTracks:-1:1
                    ...
                    startInfo(i,:) = [imageInfo.tracksX(timePoint(1),i) ...
                        imageInfo.tracksY(timePoint(1),i) imageInfo.tracksZ(timePoint(1),i) timePoint(1)];
                    endInfo(i,:) = [imageInfo.tracksX(timePoint(end),i) ...
                        imageInfo.tracksY(timePoint(end),i) imageInfo.tracksZ(timePoint(end),i) timePoint(end)];
                end
                ...
                switch colorTime
                    case {'1','2','3'}
                        ...
                        plot3(startInfo(indx,1),startInfo(indx,2),startInfo(indx,3),'k','LineStyle','none','marker','o','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                        indx = find(endInfo(:,4)>=timeRange(1) & endInfo(:,4)<=timeRange(2));
                        plot3(endInfo(indx,1),endInfo(indx,2),endInfo(indx,3),'k','LineStyle','none','marker','square','YLimInclude','off','XLimInclude','off','ZLimInclude','off');
                        %             case '2'
